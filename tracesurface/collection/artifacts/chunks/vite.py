...
class ViteChunkStrategy:
    name = "vite"

    def supports(self, source: SourceDocument) -> bool:
        return (
            VITE_FINGERPRINT in source.text
            or VITE_FINGERPRINT_ALT in source.text
            or (source.tree_root is not None and "import(" in source.text)
        )
        ...
        if VITE_FINGERPRINT in source.text or VITE_FINGERPRINT_ALT in source.text:
            for node in walk_pre_iter(tree_root):
                ...
                for child in init.named_children:
                    ...
                    if val.endswith(".js"):
                        urls.add(vite_asset_url(val, ctx.base_url))
